...
from scipy.linalg import expm_frechet, logm
...
class SPD:
    ...
    def __init__(self, k=1, d=3):
        ...
        if k == 1:
            self._name = (
                "Manifold of symmetric positive definite {d} x {d} matrices".format(
                    d=d, k=k
                )
            )
        elif k > 1:
            self._name = "Manifold of {k} symmetric positive definite {d} x {d} matrices (Sym^+({d}))^{k}".format(
                d=d, k=k
            )
        else:
