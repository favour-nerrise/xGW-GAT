...
def build_dataset(a1, args, y):
    ...
    for i in range(a1.shape[0]):
        ...
        data = Data(
            x=x1[i], edge_index=edge_index, edge_attr=edge_attr, y=y[i], adj=a1[i]
        )
...
def compute_x(a1: np.ndarray, node_feature: str):
    ...
    if node_feature == "identity":
        x = torch.cat([torch.diag(torch.ones(a1.shape[1]))] * a1.shape[0]).reshape(
            [a1.shape[0], a1.shape[1], -1]
        )
        x1 = x.clone()
